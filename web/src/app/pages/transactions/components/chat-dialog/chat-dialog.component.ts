	...
	`;
		...
		cleanResponse(response: string) {
		  ...
		  response = response.replace(regex, (match, tableName) => {
			...
			const titleCaseTableName = tableName.replace(/\b\w/g, (char : string) => char.toUpperCase());
		...
		async formulateResponse(LLM_MODEL: string, question: string, res: string, PROD: boolean) {
		  const message = `
	Question: "${question}".
	Answer: "${res}".
